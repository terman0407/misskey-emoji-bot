...
function buildEditModal(approvalKey, currentMeta) {
	...
	modal.addComponents(
		new ActionRowBuilder().addComponents(
			new TextInputBuilder().setCustomId('name').setLabel('絵文字名 (a-z 0-9 _)').setStyle(TextInputStyle.Short)
				.setRequired(true).setMaxLength(128).setValue(currentMeta.name ?? ''),
		),
		new ActionRowBuilder().addComponents(
